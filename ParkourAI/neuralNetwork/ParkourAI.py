...
#testShape = X.shape[1]
print("\n X.shape:", X.shape) 
print("\n X.shape[0]:", X.shape[0]) # gives the shape as (3,), which is strange since we have 737 rows and 4 columns
print()
...
# add layers
# I might need to change X.shape[1] to something else to account for the number of input datasets
#model.add(layer_Dense(X.shape[1], 128)) # input layer
model.add(layer_Dense(X.shape[0], 128)) # input layer
...
#image_data = cv2.imread('prediction_data/pants.png', cv2.IMREAD_GRAYSCALE)
